...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Recipe
      .findOne(_.pick(params, _.keys(Recipe.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count recipes.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Recipe.create(data);
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Recipe.associations.map(a => a.alias));
    const data = _.omit(values, Recipe.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Recipe.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Recipe.associations.map(async association => {
